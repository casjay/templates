...
#### OS Specific
test_pkg() { devnull sudo dpkg-query -l "$1" && printf_success "$1 is installed" && return 0 || return 1 ; setexitstatus ; set -- ;}
remove_pkg() { if test_pkg "$1"; then execute "sudo pkmgr remove $1" "Removing: $1"; fi ; setexitstatus ; set -- ;}
install_pkg() { if ! test_pkg "$1"; then execute "sudo pkmgr install $1" "Installing: $1"; fi ; setexitstatus ; set --  ;}
