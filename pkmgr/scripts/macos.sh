...
test_pkg() { devnull brew list $1 && printf_success "$1 is installed" && return 1 || return 0 ; setexitstatus ; set -- ;}
remove_pkg() { if ! test_pkg "$1" ; then execute "brew remove -f $1" "Removing: $1" ; fi ; setexitstatus ; set -- ;}
install_pkg() { if test_pkg "$1" ; then execute "brew install -f $1" "Installing: $1" ; fi ; setexitstatus ; set --  ;}
